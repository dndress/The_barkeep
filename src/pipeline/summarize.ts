...
function buildPrompt(opts: {
  ...
  return [
    ...
    '',
    `- short: an IN-WORLD recap suitable for a Discord post. Target around ${opts.shortWordTarget} words. Capture the main events with enough detail that a returning player can tell what happened. Written in ${lang}. Follow the IMMERSION RULES below.`,
    `- full: a detailed IN-WORLD narrative recap, as if a chronicler were retelling the tale. No length cap — scale to the session. Include specific NPC names, locations, decisions, and in-character dialogue. Written in ${lang}. Follow the IMMERSION RULES below.`,
